...
export async function downloadAttachment(id: string): Promise<{ data: ArrayBuffer; sha256: string; iv: string; contentType: string }> {
  try {
    ...
    const iv = (response.headers['x-content-iv'] as string) || '';
    const contentType = (response.headers['content-type'] as string) || 'application/octet-stream';
    return { data: response.data as ArrayBuffer, sha256, iv, contentType };
  } catch (error) {
