...
fun SettingsScreen(
    ...
    Column(modifier = Modifier.fillMaxSize().padding(padding).padding(horizontal = 16.dp)) {
        ...
        Text("当前账户: ${uiState.username}", style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.primary)
        ...
        WarmCard {
            ...
            Spacer(modifier = Modifier.height(8.dp))
            Button(onClick = viewModel::triggerSync, enabled = !uiState.isSyncing, modifier = Modifier.fillMaxWidth()) {
        ...
        WarmCard {
            ...
            Spacer(modifier = Modifier.height(8.dp))
            Button(onClick = { showRecoveryDialog = true }, modifier = Modifier.fillMaxWidth()) { Text("管理托管") }
        }
        Spacer(modifier = Modifier.height(12.dp))
        ...
        AlertDialog(
                    ...
                    OutlinedTextField(confirmPassword, { confirmPassword = it }, label = { Text("确认新密码") }, singleLine = true,
                        ...
                        trailingIcon = {
                            IconButton(onClick = { passwordVisible = !passwordVisible }) {
                                Icon(if (passwordVisible) Icons.Default.VisibilityOff else Icons.Default.Visibility, "切换可见")
                            }
                        })
                }
            },
            confirmButton = {
                TextButton(onClick = {
                    ...
                    showPasswordDialog = false
                    oldPassword = ""; newPassword = ""; confirmPassword = ""
                }, enabled = oldPassword.isNotBlank() && newPassword.isNotBlank()) { Text("确认") }
            },
            dismissButton = {
                TextButton(onClick = {
                    showPasswordDialog = false
        ...
        AlertDialog(
                ...
                Column {
                    if (uiState.hasRecovery) {
                        ...
                        OutlinedTextField(deleteAuthPassword, { deleteAuthPassword = it }, label = { Text("输入登录密码确认") }, singleLine = true,
                            ...
                            keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Password),
                            trailingIcon = { IconButton(onClick = { recoveryPasswordVisible = !recoveryPasswordVisible }) { Icon(if (recoveryPasswordVisible) Icons.Default.VisibilityOff else Icons.Default.Visibility, "切换可见") } })
                    } else {
            ...
            confirmButton = {
                TextButton(onClick = {
                    ...
                    showRecoveryDialog = false
                    recoveryPhrase = ""; deleteAuthPassword = ""
                }, enabled = deleteAuthPassword.isNotBlank()) { Text("确认") }
            },
            dismissButton = {
                TextButton(onClick = {
                    showRecoveryDialog = false
            ...
            text = {
                Column {
                    ...
                    OutlinedTextField(deleteAuthPassword, { deleteAuthPassword = it }, label = { Text("输入密码确认") }, singleLine = true,
                        ...
                        keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Password),
                        trailingIcon = { IconButton(onClick = { deletePasswordVisible = !deletePasswordVisible }) { Icon(if (deletePasswordVisible) Icons.Default.VisibilityOff else Icons.Default.Visibility, "切换可见") } })
                }
            },
            confirmButton = {
                TextButton(onClick = {
                    ...
                    showDeleteDialog = false
                    deleteAuthPassword = ""
                }, enabled = deleteAuthPassword.isNotBlank()) { Text("确认注销", color = MaterialTheme.colorScheme.error) }
            },
            dismissButton = {
                TextButton(onClick = {
                    showDeleteDialog = false
